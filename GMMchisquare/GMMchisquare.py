...
def probe_filter(datainputnorm,log2transform=False, filt=0,threshold_filter = 0.01,variance_filter = 0.0125):
    ...
    if log2transform == True:
        ...
        varp = pd.DataFrame({'variance':var, 'threshold':aap['threshold']}, index = datainput.index)
        varp = varp[(varp['variance']<(max(var)-min(var))*variance_filter) & (varp['threshold']<=math.ceil(threshold_filter*len(datainput.T))+1)] #remove sample with low variance
        ...
        varp = pd.DataFrame({'variance':var, 'threshold': aap['threshold']}, index = datainputnorm.index)
        varp = varp[(varp['variance']<(max(var)-min(var))*variance_filter) & (varp['threshold']<=math.ceil(threshold_filter*len(datainput.T))+1)] #remove sample with low variance
...
########################################## UNIMODAL CATEGORIES ##########################################
    ...
    if unimodal_categories == True and n_comp ==1:
        ...
        try:
            ...
            raise Exception('Amigo, you did not put a background filter in, use the filt variable or make unimodal_categories = False if you do not need a background filter for the GMM pipeline')
        ...
        if (posneg_ratio <= 0.95 and posneg_ratio >= 0.05) and (negpos_ratio <= 0.95 and negpos_ratio >= 0.05): 
            ...
            categories = [1 if x<=lower_bound else 2 if x>=upper_bound else np.nan for x in data]
...
#     ----------------------------IF TAIL PROBELM RUN ------------------------------------------------------------
    ...
    return categories, group_div, datas, condition, chis, rem, BICs, cclemon, biclemon, realbic, n_comp, n_components,BIC,bimodalc, xs, expected, expectedt, expectedboth, nums, count, remove, dof, means, covars
        ...
        if cell_lines != []:
            ...
            if all([isinstance(i, list) for i in cell_lines]): #check if list of list or just a list of cell lines
                ...
                if log2transform == True:
                    ...
                    print(input_datanorm[cell_lines].loc[ID])
                    n2, bins2, patches2 = ax1.hist(input_datanorm[cell_lines].loc[ID],hatch='///',facecolor='none',
                                                   bins = np.linspace(min(x_axis),max(x_axis), num=nums+1),
                                                   histtype='barstacked',alpha=0.5, ec='black')     
                    
            a2.legend(handles=[background_p, background_p2]+all_highlight, loc ='upper right') #add all labels
...
def output_bins_func(data, bins):
    index = [[bins[x],bins[x+1]] for x in range(0,len(bins)-1)] #get all pairs of bins edges
    
    final_df = [data[(data<=x[1]) & (data>=x[0])].dropna(axis=1).columns.tolist() for x in index] 
...
def GMM_modelingt(ID,input_datanormal,log2transform = True, dynamic_binning_s=True, tune_factor=0.99,filt= None, verbosity = False, graphs = True, 
    ...
    #run GMM pipeline
    if log2transform == True:
        categories, group_div, datas, condition, chis, rem, BICs, cclemon, biclemon, realbic, n_comp, n_components,BIC,bimodalc, xs, expected, expectedt, expectedboth, nums, count, remove, dof, means, covars = GMM_pipeline(data, log2transform, filt, meanf, stdf, verbosity,farinto, dynamic_binning_s, Single_tail_validation, tune_factor, chisquaremethod, unimodal_categories)
    elif log2transform == False:
        categories, group_div, datas, condition, chis, rem, BICs, cclemon, biclemon, realbic, n_comp, n_components,BIC,bimodalc, xs, expected, expectedt, expectedboth, nums, count, remove, dof, means, covars = GMM_pipeline(datanorm, log2transform, filt, meanf, stdf, verbosity,farinto, dynamic_binning_s, Single_tail_validation, tune_factor, chisquaremethod, unimodal_categories)
